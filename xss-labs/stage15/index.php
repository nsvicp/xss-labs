...
<span class="hl-comment">// 2) htmlspecialchars 转义 HTML 特殊字符（含单引号）</span>
<span class="hl-var">$filtered_js</span> = <span class="hl-fn">htmlspecialchars</span>(<span class="hl-var">$decoded</span>, <span class="hl-fn">ENT_QUOTES</span> | <span class="hl-fn">ENT_HTML5</span>, <span class="hl-str">'UTF-8'</span>);</div>
      <p>输出位置分析：</p>
...
<span class="hl-tag">&lt;script&gt;</span>document.write('<span class="hl-val">&lt;?php echo $filtered_js; ?&gt;</span>');<span class="hl-tag">&lt;/script&gt;</span></div>
      <p>document.write 将用户输入作为 JS 字符串写入页面。PHP 用 <code>str_replace("\\x", "x")</code> 去掉 <code>\x</code> 中的反斜杠（<code>\x3c</code> → <code>x3c</code>），再用 <code>htmlspecialchars</code> 转义 HTML 特殊字符。但双反斜杠 <code>\\x3c</code> 中只有第二个 <code>\x</code> 被匹配替换，第一个反斜杠保留，输出为 <code>\x3c</code>，JS 引擎会将其解码为 <code>&lt;</code>。</p>
    </div>
...
<span class="hl-fn">eval</span>(<span class="hl-js">"\x61\x6c\x65\x72\x74(1)"</span>) <span class="hl-comment">// 等价于 eval("alert(1)")</span></div>
      <p>本关用 <code>str_replace("\\x", "x")</code> 去掉反斜杠，再用 <code>htmlspecialchars</code> 转义 HTML 特殊字符。单反斜杠 <code>\x3c</code> 被完整替换为 <code>x3c</code>；但双反斜杠 <code>\\x3c</code> 中 <code>str_replace</code> 只匹配到第二个 <code>\x</code> 并替换，第一个反斜杠保留，输出为 <code>\x3c</code>，JS 引擎解码后得到字符 <code>&lt;</code>。</p>
    </div>
...
<span class="hl-comment">// 2) htmlspecialchars：无特殊 HTML 字符 → 不变</span>
...
<span class="hl-comment">// 2) htmlspecialchars：\x3c 不含 HTML 特殊字符 → 不变</span>
...
<span class="hl-comment">// 2) htmlspecialchars：</span>
<span class="hl-comment">//    &lt; → &amp;lt;，&gt; → &amp;gt;（被转义为 HTML 实体）</span>
<span class="hl-comment">// JS 执行：document.write('&amp;lt;script&amp;gt;alert(document.domain);&amp;lt;/script&amp;gt;')</span>
<span class="hl-comment">// ❌ 显示为文本 &lt;script&gt;...，不会触发 XSS</span></div>
      <p>这是因为 <code>&lt;</code> 和 <code>&gt;</code> 被 <code>htmlspecialchars</code> 转义为 HTML 实体。单反斜杠 <code>\x3c</code> 也不行——<code>\x</code> 被替换为 <code>x</code> 后变成 <code>x3c</code>，JS 引擎只看到纯文本。</p>
...
<span class="hl-comment">// 等价于 &lt;script&gt;alert(document.domain)&lt;/script&gt;</span></div>
    ...
    <div class="knowledge-item">
      ...
      <p>本关的核心教训：<strong>简单的子串替换无法防御编码层绕过</strong>。<code>str_replace("\\x", "x")</code> 看似能清除十六进制转义，但忽略了双反斜杠 <code>\\x</code> 中只有第二个 <code>\x</code> 会被匹配。正确的做法是使用正则 <code>preg_replace('/\\\\+x/i', 'x')</code> 匹配任意数量的前导反斜杠，或者对输出统一使用 <code>json_encode()</code> 编码。</p>
      <div class="code-block"><span class="hl-comment">// 更安全的做法</span>
<span class="hl-comment">// 方式1：用正则匹配所有反斜杠变体</span>
<span class="hl-var">$decoded</span> = <span class="hl-fn">preg_replace</span>(<span class="hl-str">'/\\\\+x/i'</span>, <span class="hl-str">'x'</span>, <span class="hl-var">$p1_raw</span>); <span class="hl-comment">// \x \\x \\\x 全部 → x</span>

<span class="hl-comment">// 方式2：去掉所有反斜杠</span>
...
<span class="hl-comment">// 方式3：使用 json_encode（推荐）</span>
