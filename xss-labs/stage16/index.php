 ...
 *     1) htmlentities 编码 HTML 特殊字符
 *     2) preg_replace /\\+x/i → x：去掉所有 \x 前的反斜杠（封堵十六进制绕过）
 *     3) str_replace \u → u：去掉反斜杠，阻止 Unicode 转义
...
// 1) htmlentities 编码
// 2) preg_replace('/\\\\+x/i', 'x', $data) 去掉所有 \x 前的反斜杠
// 3) str_replace('\u','u',$data) 过滤 Unicode 转义
$decoded = htmlentities($p1_raw);
$decoded = preg_replace('/\\\\+x/i', 'x', $decoded); // 去掉所有 \x 前的反斜杠（\x \\x \\\x 全部 → x）
...
<main>
  ...
  <div class="knowledge-section">
    ...
    <div class="knowledge-item">
      <h3>后端 PHP 漏洞代码</h3>
      <p>本关基于 Stage 15 升级了防御，同时封堵 <code>\x</code> 十六进制和 <code>\u</code> Unicode 转义：</p>
...
<span class="hl-comment">// 1) htmlentities 编码 HTML 特殊字符</span>
<span class="hl-var">$decoded</span> = <span class="hl-fn">htmlentities</span>(<span class="hl-var">$p1_raw</span>);
<span class="hl-comment">// 2) preg_replace('/\\\\+x/i', 'x') 去掉所有 \x 前的反斜杠</span>
<span class="hl-comment">//    \x3c → x3c，\\x3c → x3c，\\\x3c → x3c（全部失效）</span>
<span class="hl-var">$decoded</span> = <span class="hl-fn">preg_replace</span>(<span class="hl-str">'/\\\\+x/i'</span>, <span class="hl-str">'x'</span>, <span class="hl-var">$decoded</span>);
<span class="hl-comment">// 3) str_replace('\u', 'u') 过滤 Unicode 转义</span>
...
<span class="hl-var">$filtered_js</span> = <span class="hl-var">$decoded</span>;</div>
      <p>输出位置分析：</p>
...
<span class="hl-tag">&lt;script&gt;</span>document.write('<span class="hl-val">&lt;?php echo $filtered_js; ?&gt;</span>');<span class="hl-tag">&lt;/script&gt;</span></div>
      <p>本关用 <code>preg_replace('/\\\\+x/i', 'x')</code> 一次性封堵了所有 <code>\x</code> 变体（单反斜杠、双反斜杠、三反斜杠...），同时用 <code>str_replace('\u','u')</code> 过滤 <code>\u</code> Unicode 转义。看起来两种 JS 转义都被堵死了，但 <code>str_replace</code> 是<strong>精确子串匹配</strong>，攻击者输入 <code>\\u003c</code>（双反斜杠）时，<code>\u</code> 前面多了一个 <code>\</code>，过滤只吃掉一个反斜杠，结果反而变成有效的 <code>\u003c</code>。</p>
    </div>

    <div class="knowledge-item">
      <h3>例1：\x 绕过被彻底封堵</h3>
      <p>无论输入单反斜杠还是双反斜杠，<code>\x3c</code> 全部失效：</p>
      <div class="code-block"><span class="hl-comment">// 输入：\x3cscript\x3ealert(document.domain);\x3c/script\x3e</span>
<span class="hl-comment">// 1) htmlentities：无特殊 HTML 字符 → 不变</span>
<span class="hl-comment">// 2) preg_replace('/\\\\+x/i', 'x')：</span>
<span class="hl-comment">//    匹配 \x3c → 替换为 x3c</span>
<span class="hl-comment">// 3) str_replace('\u', 'u')：无 \u → 不变</span>
<span class="hl-comment">// 回显：document.write('x3cscriptx3ealert(document.domain);x3c/scriptx3e')</span>
<span class="hl-comment">// ❌ 不会触发 XSS</span>

<span class="hl-comment">// 输入：\\x3cscript\\x3ealert(document.domain);\\x3c/script\\x3e</span>
<span class="hl-comment">// 1) htmlentities：不变</span>
<span class="hl-comment">// 2) preg_replace('/\\\\+x/i', 'x')：</span>
<span class="hl-comment">//    匹配 \\x → 替换为 x</span>
<span class="hl-comment">// 3) str_replace('\u', 'u')：无 \u → 不变</span>
<span class="hl-comment">// 回显：document.write('x3cscriptx3ealert(document.domain);x3c/scriptx3e')</span>
<span class="hl-comment">// ❌ 同样失效，Stage 15 的 Payload 在本关不可用</span></div>
      <p><code>preg_replace('/\\\\+x/i', 'x')</code> 中的 <code>\\\\+</code> 匹配一个或多个反斜杠后跟 <code>x</code>，统一替换为 <code>x</code>。无论几个反斜杠都会被清除。</p>
    </div>
...
<span class="hl-comment">// 1) htmlentities：不变</span>
<span class="hl-comment">// 2) preg_replace('/\\\\+x/i', 'x')：无 \x → 不变</span>
<span class="hl-comment">// 3) str_replace('\u', 'u')：</span>
...
<span class="hl-comment">// ❌ 不会触发 XSS，防御生效</span></div>
      <p><code>str_replace('\u','u')</code> 精确匹配 <code>\u</code> 两个字节，把反斜杠吃掉。JS 引擎看到的是普通文本 <code>u003c</code>。</p>
    </div>
...
<span class="hl-comment">// 浏览器重新解析 HTML，script 标签执行 ✅</span></div>
      <p>关键原理：输入 <code>\\u003c</code>（2个反斜杠），<code>str_replace('\u','u')</code> 匹配到第2个反斜杠开头的 <code>\u</code>，替换后只剩1个反斜杠，变成了 <code>\u003c</code>——恰好是 JS 能解码的 Unicode 转义！<strong>过滤操作反而帮了忙</strong>，把双反斜杠"修剪"成了单反斜杠。</p>
      <p>注意对比：本关对 <code>\x</code> 用了 <code>preg_replace('/\\\\+x/i', 'x')</code> 一次性匹配所有反斜杠数量，所以 <code>\\x</code> 无法绕过；但对 <code>\u</code> 用的是 <code>str_replace</code>，只能精确匹配单反斜杠 <code>\u</code>，这就是双反斜杠 <code>\\u</code> 能绕过的原因。</p>
    </div>
...
<span class="hl-comment">// 等价于 &lt;script&gt;alert(document.domain)&lt;/script&gt;</span></div>
    ...
    <div class="knowledge-item">
      <h3>互动思考 💬</h3>
      <p><strong>问：</strong>如果将 <code>\u</code> 的过滤也改为和 <code>\x</code> 一样的正则 <code>preg_replace('/\\\\+u/i', 'u')</code>，双反斜杠绕过还能生效吗？</p>
      <p><strong>答：</strong>不能。正则 <code>/\\\\+u/i</code> 中的 <code>\\\\+</code> 匹配一个或多个反斜杠，<code>\\u003c</code> 会被整体匹配并替换为 <code>u003c</code>，无论几个反斜杠都会被清除。本关的核心教训：<strong>str_replace 做子串替换时，无法处理"双反斜杠"的情况</strong>，应统一使用正则 <code>/\\\\+x/i</code> 和 <code>/\\\\+u/i</code> 来匹配任意数量的前导反斜杠。更根本的做法是<strong>不要将用户输入直接拼接到 JS 代码中</strong>，应使用 <code>json_encode()</code> 对数据进行 JSON 编码后再输出。</p>
      <div class="code-block"><span class="hl-comment">// 更安全的过滤方式</span>
<span class="hl-var">$decoded</span> = <span class="hl-fn">preg_replace</span>(<span class="hl-str">'/\\\\+(x|u)/i'</span>, <span class="hl-str">'$1'</span>, <span class="hl-var">$decoded</span>); <span class="hl-comment">// 一条正则封堵 \x 和 \u</span>
